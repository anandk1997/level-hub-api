 ...
 * @param {string} mailData.email
 */
const sendFeedbackMail = async (mailData) => {
	try {
		const mailer = new Mailer();

		let mailText = `Hi ${mailData.fullName} \n\nGood news — ${mailData.feedback} has just accepted your invitation and successfully created their account.`;
		...
		const mailDetails = {
			to: mailData.email,
			subject: `${mailData.fullName} Has Accepted Your Invitation`, // Subject line
			text: mailText, // plain text body
			html: mailHtml, // html body
			priority: "high",
			useTemplate: true,
